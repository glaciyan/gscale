...
export function electroTravelerTalentCost() {
    const ig = new ItemGen({
        common: ItemGroup.handguard,
        weekly: items.dragon_lords_crown.name,
    });

    const ballad = new ItemGen({ book: ItemGroup.light });
    const freedom = new ItemGen({ book: ItemGroup.transience });
    const resistance = new ItemGen({ book: ItemGroup.elegance });
